...
def draw_matching(sample_info, select_name, features_type, features=None):
    ...
    for species in features:
        ...
        y_points = []
        y_error = [[], []]
        print(sample_info[[species, 'mouths']])
        for i in range(1, max(y)+1):
            if i in xtick and len(sample_info[sample_info['mouths'] == i][species]) > 1:
                sample_info_quantile = sample_info[sample_info['mouths'] == i][species].quantile([0.25, 0.5, 0.75])
                print(species, i)
                print(sample_info_quantile)
                x_points.append(i)
                y_points.append(sample_info_quantile.iloc[1])
                y_error[0].append(sample_info_quantile.iloc[1] - sample_info_quantile.iloc[0])
                y_error[1].append(sample_info_quantile.iloc[2] - sample_info_quantile.iloc[1])
        ax.errorbar(x_points, y_points, yerr=y_error)
